...
def plot_tc_evolution(
    ...
    fig.suptitle(
        "Texture Coefficient Evolution by Peak", fontsize=14, fontweight="bold", y=1.02
    )
    plt.tight_layout()
...
def plot_texture_fraction_single(
    ...
    all_hkls = sorted(
        set(hkl for s in high_quality_data for hkl in s.get("tc_values", {}).keys())
    )
    ...
    hkl_styles = {
        hkl: line_styles[i % len(line_styles)] for i, hkl in enumerate(all_hkls)
    }
    ...
    if metric == "tc":
        all_tc_vals = [
            v for s in high_quality_data for v in s.get("tc_values", {}).values()
        ]
        if all_tc_vals:
            global_max_tc = max(
                max(all_tc_vals) * 1.15, 1.6
            )  # Ensure at least 1.6 if data is high
    ...
    for idx, conc in enumerate(concentrations):
        ...
        x_label = (
            "Annealing Time (hours)" if x_param == "time" else "Concentration (mL/1.5L)"
        )
    ...
    fig.suptitle(
        f"{title_metric} Comparison by Concentration",
        fontsize=16,
        fontweight="bold",
        y=0.995,
    )
