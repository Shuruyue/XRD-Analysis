...
def generate_sample_fitting_plot(
    ...
    for idx, (hkl, expected_pos) in enumerate(PEAK_POSITIONS.items()):
        ...
        fit_result = fit_peak_with_diagnosis(
            two_theta, intensity, expected_pos, use_doublet=True
        )

        if fit_result["theta_range"] is not None:
            # Plot original data
            ax.scatter(
                fit_result["theta_range"],
                fit_result["int_range"],
                s=15,
                alpha=0.5,
                color="gray",
                label="Data",
            ...
            if fit_result["success"] and fit_result["fitted_curve"] is not None:
                ...
                if not np.isnan(center_ka2):
                    ...
                    if fit_result.get("method") in ["doublet", "doublet-true-voigt"]:
                        ...
                        y_ka2 = TrueVoigt.profile(
                            x_vals, center_ka2, amp_ka2, sigma, gamma
                        )
...
def main():
    ...
    for filepath in txt_files:
        ...
        if result:
            all_results.append({"filename": filepath.name, "peaks": result})
            print("  [OK] Saved (DPI: 1000)")
        else:
            print("  ✗ Failed")
