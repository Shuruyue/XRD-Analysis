...
def plot_peak_fit(
    ...
    ax1.scatter(
        two_theta, intensity_obs, s=20, c="black", alpha=0.5, label="Observed", zorder=3
    )
    ax1.plot(
        two_theta,
        intensity_fit,
        ...
        linewidth=2.5,
        label="Fitted (Pseudo-Voigt)",
        zorder=4,
    )

    # Fill area under fit
    ax1.fill_between(two_theta, 0, intensity_fit, alpha=0.2, color=COLORBLIND_SAFE[0])
...
def plot_doublet_comparison(
    ...
    ax1 = axes[0, 0]
    ax1.scatter(two_theta, intensity_obs, s=10, c="black", alpha=0.4, label="Observed")
    ax1.plot(
        two_theta,
        intensity_single,
        color=COLORBLIND_SAFE[1],
        linewidth=2,
        linestyle="--",
    ...
    ax2.plot(
        ...
        color=COLORBLIND_SAFE[1],
        alpha=0.7,
        label=f"Single (RMSE={np.sqrt(np.mean(res_single**2)):.1f})",
    )
    ax2.plot(
        two_theta,
        res_doublet,
        color=COLORBLIND_SAFE[0],
        alpha=0.7,
        label=f"Doublet (RMSE={np.sqrt(np.mean(res_doublet**2)):.1f})",
    )
    ...
    bars = ax3.bar(
        methods, r2_values, color=colors, alpha=0.8, edgecolor="black", linewidth=1
    )
    ...
    ax3.axhline(y=0.95, color="green", linestyle="--", alpha=0.7, label="Target (0.95)")
    ...
    table_data = [
        ...
        [
            "Single Peak",
            f"{r2_single:.4f}",
            f"{np.sqrt(np.mean(res_single**2)):.1f}",
            "-",
        ],
        [
            "Kα Doublet",
            f"{r2_doublet:.4f}",
            f"{np.sqrt(np.mean(res_doublet**2)):.1f}",
            (
                f"{(r2_doublet - r2_single) / r2_single * 100:+.2f}%"
                if r2_single > 0
                else "N/A"
            ),
        ],
...
def plot_fit_residuals(
    ...
    if len(residuals) > 10:
        ...
        ax1.plot(
            x_rolling, rolling_mean, color="orange", linewidth=2, label="Rolling mean"
        )
