...
if __name__ == "__main__":
    ...
    print(
        f"  η=0.3 (Gaussian-dominated): factor = {get_pv_integration_factor(0.3):.4f}"
    )
    print(
        f"  η=0.5 (balanced):           factor = {get_pv_integration_factor(0.5):.4f}"
    )
    print(
        f"  η=0.7 (Lorentzian-dominated): factor = {get_pv_integration_factor(0.7):.4f}"
    )
    print(
        f"Using fixed 1.0645 for Lorentzian (eta=1) causes {abs((1.0645 - get_pv_integration_factor(1.0))/get_pv_integration_factor(1.0)*100):.1f}% error!"
    )
