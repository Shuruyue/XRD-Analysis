...
class LMOptimizer:
    ...
    def fit_single_peak(
        ...
        if initial_guess is None:
            if peak_idx is not None:
                initial_guess = self._auto_initial_guess(two_theta, intensity, peak_idx)
            else:
                # Use maximum as peak
        ...
        try:
            ...
            params = PseudoVoigtParams(
                center=popt[0], amplitude=popt[1], fwhm=popt[2], eta=popt[3]
            )
            ...
            logger.debug("Primary fit failed, trying fallback: %s", e)
    ...
    def _fallback_fit(
        ...
        try:
            ...
            p0 = [
                initial_guess.center,
                initial_guess.amplitude - bg,
                initial_guess.fwhm,
                initial_guess.eta,
            ]
            bounds = ([two_theta.min(), 0, 0.01, 0], [two_theta.max(), np.inf, 5.0, 1])

            popt, pcov = curve_fit(
                simple_pv, two_theta, intensity, p0=p0, bounds=bounds, maxfev=500
            )
        ...
        except (ValueError, RuntimeError, np.linalg.LinAlgError) as e2:
            return FitResult(
    ...
    def fit_voigt(
        ...
        p0 = [
            center_init,
            amplitude_init - (slope_init * center_init + intercept_init),
            sigma_init,
            gamma_init,
            slope_init,
            intercept_init,
        ...
        try:
            ...
            params = PseudoVoigtParams(
                center=popt[0], amplitude=popt[1], fwhm=fwhm_total, eta=eta_equiv
            )
            ...
            logger.debug("Voigt fit failed, falling back to Pseudo-Voigt")
        ...
        if initial_guesses:
            for guess in initial_guesses:
                ...
                result = self.fit_single_peak(
                    two_theta[mask], intensity[mask], initial_guess=guess
                )
