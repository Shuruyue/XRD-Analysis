...
from .data_loader import load_xrd_data
...
class PreprocessingPipeline:
    ...
    def _init_processors(self):
        ...
        self.smoother = SavitzkyGolayFilter(
            window_size=self.window_size, poly_order=self.poly_order
        )
        self.background_subtractor = BackgroundSubtractor(
            method=self.background_method, poly_degree=self.background_degree
        )
    ...
    def run(self, two_theta: np.ndarray, intensity: np.ndarray) -> PreprocessingResult:
