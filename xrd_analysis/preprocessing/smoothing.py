...
class SavitzkyGolayFilter:
    ...
    def apply(self, intensity: np.ndarray) -> np.ndarray:
        ...
        return savgol_filter(
            intensity, window_length=self.window_size, polyorder=self.poly_order
        )

    def apply_derivative(self, intensity: np.ndarray, deriv: int = 1) -> np.ndarray:
...
def smooth_xrd_data(
    intensity: np.ndarray, window_size: int = 11, poly_order: int = 3
) -> np.ndarray:
