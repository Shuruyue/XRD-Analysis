...
class ErrorAnalyzer:
    ...
    def validate_broadening(
        self, fwhm_observed: float, fwhm_instrumental: float
    ) -> ValidationResult:
...
def validate_size_range(
    size_nm: float, min_size: float = 2.0, max_size: float = 200.0
) -> bool:
...
def check_broadening_ratio(
    fwhm_observed: float, fwhm_instrumental: float, min_ratio: float = 1.2
) -> bool:
