...
def test_prepare_wh_input_uses_scherrer_corrected_fwhm():
    ...
    scherrer_results = [
        ScherrerResult(
            size_nm=20.0, size_angstrom=200.0, two_theta=43.3, fwhm_sample=0.21
        ),
        ScherrerResult(
            size_nm=21.0, size_angstrom=210.0, two_theta=50.4, fwhm_sample=0.19
        ),
        ScherrerResult(
            size_nm=22.0, size_angstrom=220.0, two_theta=74.1, fwhm_sample=0.17
        ),
    ]

    two_theta_arr, fwhm_arr, hkl_list = pipeline._prepare_wh_input(
        peaks, scherrer_results
    )
